...
class Backbone(nn.Module):
    def __init__(self, hidden_size):
        ...
        from clip_openAI_clone.open_clip_280.src import open_clip

        backbone, _, preprocess = open_clip.create_model_and_transforms('ViT-H-14')
        ...
        split_index = int(len(param_names) * 1 / 2)
...
def main(args):
    ...
    backbone = torch.nn.parallel.DistributedDataParallel(
        module=backbone, broadcast_buffers=False, device_ids=[local_rank], bucket_cap_mb=8,
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels) in enumerate(train_loader):
            ...
            if cfg.save_all_states and global_step % 500 == 0:
                checkpoint = {
                    "epoch": epoch + 1,
